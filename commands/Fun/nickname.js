...
module.exports = {
    ...
    callback: async(message, args, client) => {
        ...
        const timeout = 1 // 1 Hour In MiliSecond // For CoolDown
...
Default CoolDown Is **1 Hour**
    ...
    React With <:Yes:802970837065924629> To Change Name
    React With <:No:802970836498907179> To Cancel Request
            `)
            message.channel.send(embed).then(message => {
                message.react('<:Yes:802970837065924629>') // React To Embed With Yes // Change Emoji
                message.react('<:No:802970836498907179>') // React To Embed With No // Change Emoji
                ...
                collector.on('collect', async r => {
                    switch (r.emoji.name) {
                        case 'Yes': // If User Reacted With Yes
                            member.setNickname(name)
                            message.channel.send(`Successfully Changed NickName To **${name}**`)
                            db.set(`name-time_${message.guild.id}_${member.id}`, Date.now())
                            collector.stop()
                            break;
                            case 'No': // If User Reacted With No
