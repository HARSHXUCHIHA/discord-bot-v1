    ...
    client.on('message', message => {
        ...
        if(message.channel.id === channel){
            if(message.attachments.size > 0) return message.reply('I Cant read Images') // If Images Are Sent
                ...
                fetch(`http://api.brainshop.ai/get?bid=158724&key=14jeLSIFCZzhI5Wo&uid=0&msg=${encodeURIComponent(message)}`).then(res => res.json()) // Get API URL From https://brainshop.ai/ // Dont Show AnyOne API URL, Its Same As Token For BOT
                .then(data => {
